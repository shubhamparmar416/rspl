...
class UserController extends Controller
{
    ...
    public function transaction()
    {
        $data = array();
        $data['user'] = Auth::user();
        $data['transactions'] = Transaction::whereUserId(auth()->id())->orderBy('id', 'desc')->paginate(20);
        return $this->success($data, '');
        ...
        if ($request->cpass) {
            if (Hash::check($request->cpass, $user->password)) {
                if ($request->newpass == $request->renewpass) {
                    ...
                    return $this->error([0 => 'Confirm password does not match.']);
                }
            } else {
                return $this->error([0 => 'Current password Does not match.']);
            }
        }
        $user->update($input);
    ...
    public function createTwoFactor(Request $request)
    {
        ...
        if ($oneCode == $request->code) {
            ...
            $user->twofa = 1;
            $user->save();
            return $this->success([], 'Two factor authentication activated.');
        } else {
            return $this->error([0 => 'Something went wrong!']);
        }
    }
